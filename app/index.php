			...
			*/
			...
			if(($message=="Заполнить")||($message=="заполнить")){//если пользователь хочет заполнить анкету
				...
				if($r==0) {//если ответ, который нужен уже найден и все необходимые действия выполнены
					...
					//получение истории сообщений и выбор предпоследнего
					$pre_message=json_decode(file_get_contents('https://api.vk.com/method/messages.getHistory?'.$get_params));
					
					switch ($pre_message->response->items[0]->body) {
						case 'Напиши свои любимые жанры фильмов'://если предыдущее сообщение "Напиши свои любимые жанры фильмов."
						...
						case 'Теперь напиши дату своего рождения в формате ДД.ММ.ГГГГ(день.месяц.год)   *пиши реальную дату, так как от этого зависит то, с кем ты познакомишься':
						...
						case 'Такой даты рождения не может быть (она должна быть в формате ДД.ММ.ГГГГ(день.месяц.год) напиши дату своего рождения':
						...
						case 'Напиши название города, в котором ты живешь':
						...
						case 'Напиши свои интересы, хобби, увлечения, чем ты любишь заниматься?':
						...
						case 'От какого возраста ты хочешь найти собеседника?':
							
							//возраст от 13 до 80 и ввести можно только числа
							if (!preg_match_all("/[0-9]+/",$message)||((int)$message<13)||((int)$message>79)) {
								$otwet="Такого возраста может быть в сообществе. Введи другое.";
							}
							else{
								//добавление в базу от какого возроста пользователь хочет найти пару
								update("$userId","$message","vozrast_ot");
								$otwet="До какого возраста ты хочешь найти собеседника?";
							}

							break;

						case 'Такого возраста может быть в сообществе. Введи другой возраст.':
						...
						case 'До какого возраста ты хочешь найти собеседника?':
							
							//возраст от которого может быть партнёр от 14 до 81 и ввести можно только числа
							if (!preg_match_all("/[0-9]+/",$message)||((int)$message<14)||((int)$message>80)) {
								$otwet="Такого возраста может быть в сообществе. Введи другой возраст.";
							}
							else{
								//добавление до какого возроста пользователь хочет найти пару в базу
								update("$userId","$message","vozrast_do");
